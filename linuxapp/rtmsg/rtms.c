/*************************************************************************
 ...
 > Created Time: Mon 15 May 2017 02:40:59 PM CST
 ************************************************************************/
...
//#if 1
	if (tb[RTA_METRICS])
	{
		metric = RTA_DATA(tb[RTA_METRICS]);
	}
//#else
	if(tb[RTA_GATEWAY])
	{
...
//	memcpy(dest2, dest, 4);
	if(dest)
	printf("dest:  %s\t", inet_ntoa(*(unsigned int*)dest));
//	printf("dest:  %c\t", dest2[1]);
//	printf("dest:  %c\t", dest2[2]);
//	printf("dest:  %c\t", dest2[3]);
	if(metric)
		printf("metric:  %x", *(unsigned int *)metric);
	if(gate)
	printf("gate:  %s", inet_ntoa(*(unsigned int *)gate));
	printf("\n");
#endif
	return 1;
}
