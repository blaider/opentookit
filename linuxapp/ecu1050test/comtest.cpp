...
//	old = time(NULL);
	...
	if(strncmp(MSG,buf,sizeof(MSG)) != 0){
		sprintf(result," fail");
		errors++;
//		LOGEX("test %s to %s %d %d %d %c %s",coma,comb, speed, databits, stopbits, parity,result);
	}else{
		sprintf(result," OK");
	}
...
{
	size_t speedIdx, wordIdx, stopIdx, parityIdx;
	...
	time_new = time(NULL);
	printf("speedtest %s and %s cost %5ld seconds\n",coma,comb, time_new - time_old);
	LOGEX("speedtest %s and %s cost %5ld seconds",coma,comb, time_new - time_old);
...
int fileTest(char * coma,char * comb,const char *filename)
{
	...
	time_new = time(NULL);
	long cost = time_new - time_old;
	float rate = 0;
	if(cost > 1)
		rate = 1.0*idx/cost;
	else
		rate = idx;
	LOGEX("send file %s from %s to %s cost %5ld seconds,%10.2lf bytes per second",filename,coma,comb,cost,rate);
	fclose(fileSend);
	fclose(fileRecv);
	close(fd1);
	close(fd2);
	...
	{
		LOGEX("file %s and %s are the same", filename, fileRecvName);
	}
	else
	{
		LOGEX("%s", line);
...
int selfTest(char * coma,const char *filename)
{
	...
	if(cost > 1)
		...
		rate = idx;
	LOGEX("send file %s use %s cost %5ld seconds,%10.2lf bytes per second",filename,coma,cost,rate);
	fclose(fileSend);
	fclose(fileRecv);
	close(fd1);
...
{
	return pairTest(coma,comb,115200,8,1,'n');
}
