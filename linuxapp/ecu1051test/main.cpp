...
int check_sd_card(int count)
{
	int ram_count = 1;
	if(access("/dev/mmcblk0p1",F_OK))
		return -1;
	int fd = open("/media/mmcblk0p1/sdcount",O_RDWR|O_CREAT);
	if(fd <= 0)
	{
		LOGEX("/media/mmcblk0p1/sdcount open failed!\n");
		return -EINVAL;
	}
	if(count != 1 )
	{
	...
	write(fd,&count,4);
	close(fd);
	LOGEX("sd card count=%d,count=%d,",ram_count,count);
	if((ram_count != (count - 1)) && count != 1 )
	{
		LOGEX("sd card test failed!\n");
		return -1;
	}

	LOGEX("sd card test OK!\n");
	return 0;
}
...
//	if(comtest((char *)"/dev/ttyAP0",(char *)"/dev/ttyAP1"))
//		errors++;
