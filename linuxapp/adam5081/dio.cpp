...
static int check_device()
{
	...
	if (strcmp(module_name, "ADAM5081") != 0)
		return -1;

	//adam5069 check
	if (SYS_GetModuleID(myprocHandle, SLOT_5055S, &module_id) != Success)
		return -1;
	if (SYS_GetModuleName(myprocHandle, SLOT_5055S, module_name) != Success)
		return -1;

	if (strcmp(module_name, "ADAM5055") != 0)
		return -1;
/*
	//adam5024 check
	if (SYS_GetModuleID(myprocHandle, SLOT_5081, &module_id) != Success)
...
static int set_pulse(uint32 checkvalue)
{
	...
	bool bval;
	uint32 lval = 0;
	do_set_value(SLOT_5055S,DO_CHANNEL,true);
	usleep(100*1000);
/*	di_get_value(SLOT_5081,0,&bval);
	if(bval == true)
	{
		errors++;
...
*/
	...
	do_set_value(SLOT_5055S,DO_CHANNEL,false);
	usleep(100*1000);
...
int didotest()
{
	...
	if(ret)
	{
		LOGEX("device slot check failed ,please checi first\n");
		return ret;
	}
...
int setdo()
{
	DO_SetValues(myprocHandle, SLOT_5055S, 0xffff);
