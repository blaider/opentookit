...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		if (iw_get_ext(skfd, ifname, SIOCGIWSTATS, &wrq) >= 0)
			printf("qulit:%d,level:%d,noise:%d\n", stats.qual.qual,
					(signed char) stats.qual.level, stats.qual.noise);
		else
			perror("stats");
		/* Get range info if we can */
		if (iw_get_range_info(skfd, ifname, &(range)) >= 0)
			printf("qu:%d,level:%d,noise:%d\n", range.max_qual.qual,
					(signed char) range.max_qual.level, range.max_qual.noise);
		else
				perror("max");
		sleep(1);
	}
